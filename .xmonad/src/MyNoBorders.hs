...
-- singleton in Data.List since base 4.15
...
-- |
--
-- Based on xmonad-contrib/XMonad/Layout/NoBorders.hs
-- Pretty much the copy of the original with a single edit marked as "THE CHANGE"
--
-- Modified OnlyFloat leaves borders on floating windows if there are tiled
-- windows on the focused screen. Haven't been tested with multi-screen setups.
--
...
instance (Read p, Show p, SetsAmbiguous p) => LayoutModifier (ConfigurableBorder p) Window where
  ...
  redoLayout cb@(ConfigurableBorder gh ah nh ch) lr mst wrs = do
    let gh' wset =
          let lh = hiddens gh wset lr mst wrs
...
data With
  = -- | uses 'Data.List.union'
    Union
  | -- | uses 'Data.List.\\'
    Difference
  | -- | uses 'Data.List.intersect'
    Intersection
  deriving (Read, Show)
